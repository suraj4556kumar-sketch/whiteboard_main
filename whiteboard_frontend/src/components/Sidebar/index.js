...
const Sidebar = () => {
  ...
  const fetchCanvases = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/canvas/list`, {
  ...
  const handleCreateCanvas = async () => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/canvas/create`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      console.log(response.data)  
      fetchCanvases();
  ...
  const handleDeleteCanvas = async (id) => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_URL}/canvas/delete/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchCanvases();
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_API_URL}/canvas/share/${canvasId}`,
        { email },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
